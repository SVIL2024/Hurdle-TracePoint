...
class DistanceAdj(Module):
    ...
    def forward(self, batch_size, max_seqlen, device=None, dtype=None):
        # Keep the adjacency on the same device and in the same precision as features.
        device = device or self.sigma.device
        dtype = dtype or self.sigma.dtype
        self.arith = np.arange(max_seqlen).reshape(-1, 1)
        dist = pdist(self.arith, metric='cityblock').astype(np.float32)
        self.dist = torch.from_numpy(squareform(dist)).to(device=device, dtype=dtype)
        scale = torch.exp(torch.tensor(1., device=device, dtype=dtype))
        self.dist = torch.exp(-self.dist / scale)
