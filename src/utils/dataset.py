...
class UCFDataset(data.Dataset):
    def __init__(self, clip_dim: int, file_path: str, test_mode: bool, label_map: dict, normal: bool = False, return_spans: bool = False):
    ...
    def __getitem__(self, index):
        ...
        if self.test_mode == False:
            if self.return_spans:
                clip_feature, clip_length, spans = tools.process_feat_with_spans(clip_feature, self.clip_dim)
            else:
                clip_feature, clip_length = tools.process_feat(clip_feature, self.clip_dim)
        else:
            if self.return_spans:
                clip_feature, clip_length, spans = tools.process_split_with_spans(clip_feature, self.clip_dim)
            else:
        ...
        if self.return_spans:
            return clip_feature, clip_label, clip_length, torch.tensor(spans)
        return clip_feature, clip_label, clip_length

class XDDataset(data.Dataset):
    def __init__(self, clip_dim: int, file_path: str, test_mode: bool, label_map: dict, return_spans: bool = False):
        self.df = pd.read_csv(file_path)
        self.clip_dim = clip_dim
        self.test_mode = test_mode
        self.label_map = label_map
