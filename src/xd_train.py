...
def train(model, train_loader, test_loader, args, label_map: dict, device):
    ...
    def evaluate(epoch, global_step, train_metrics):
        ...
        primary_metric = resolve_selection_metric(metrics, primary_metric_name)
        metrics.update({
            'epoch': epoch + 1,
    ...
    for e in range(start_epoch, args.max_epoch):
        ...
        for i, item in enumerate(train_loader):
            step = 0
            if args.tracepoint:
                visual_feat, text_labels, feat_lengths, visual_spans = item
                visual_spans = visual_spans.to(device)
            else:
            ...
            primary_loss = loss1 + loss2 + loss3 * 1e-4
            ...
            should_eval = args.eval_every_steps > 0 and (i + 1) % args.eval_every_steps == 0
            if should_eval:
        ...
        completed_steps = len(train_loader)
        epoch_metrics = {
        ...
        # The upstream XD protocol evaluates on the test split at every epoch and
        # selects the checkpoint by the legacy text branch AP (AP2).
        evaluate(e, completed_steps * train_loader.batch_size, epoch_metrics)
...
if __name__ == '__main__':
    ...
    configure_run_paths(args, 'xd')

    label_map = dict({'A': 'normal', 'B1': 'fighting', 'B2': 'shooting', 'B4': 'riot', 'B5': 'abuse', 'B6': 'car accident', 'G': 'explosion'})

    train_dataset = XDDataset(args.visual_length, args.train_list, False, label_map, return_spans=args.tracepoint)
    data_order_generator = None
    if args.data_order_seed is not None:
        data_order_generator = torch.Generator()
        data_order_generator.manual_seed(args.data_order_seed)
